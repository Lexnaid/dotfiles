...
-- Tema poimandres con fondo púrpura personalizado - IMPROVED
...
function M.setup()
  ...
  poimandres.setup {
    ...
    disable_background = false, -- Cambiado a false para que funcione mejor con nuestros backgrounds
  ...
  -- Esperar un poco para que se cargue completamente y aplicar purple
  vim.defer_fn(function()
    M.apply_purple_highlights()
  end, 50)
  
  -- Limpiar autocmds previos para evitar conflictos
  vim.api.nvim_del_augroup_by_name("PoimandresPurple", true)
  
  -- Crear autocmd para replicar en recarga con grupo específico
  local augroup = vim.api.nvim_create_augroup("PoimandresPurple", { clear = true })
  vim.api.nvim_create_autocmd("ColorScheme", {
    group = augroup,
    pattern = "poimandres",
    callback = function()
      -- Solo aplicar si estamos en el tema púrpura
      if vim.g.current_theme == "poimandres-purple" then
        vim.defer_fn(function()
          M.apply_purple_highlights()
        end, 10)
      end
  ...
  -- Comando para ajustar púrpura (recrear por si acaso)
  pcall(vim.api.nvim_del_user_command, 'PurpleIntensity')
